...
async function onRequestGet({ request, env: env2 }) {
  const clientId = env2.GH_OAUTH_CLIENT_ID;
  if (!clientId) {
    return new Response(
  ...
  const scope = env2.GH_OAUTH_CLIENT_SCOPE || "repo";
...
async function onRequestGet2({ request, env: env2 }) {
  ...
  let error3 = null;
  if (!code || !stateOk) {
    error3 = "Invalid OAuth request (missing code or bad state).";
  } else {
    const redirectUri = `${url.origin}/oauth/callback`;
    try {
      const resp = await fetch("https://github.com/login/oauth/access_token", {
        ...
        body: new URLSearchParams({
          client_id: env2.GH_OAUTH_CLIENT_ID,
          client_secret: env2.GH_OAUTH_CLIENT_SECRET,
          code,
          redirect_uri: redirectUri
        })
      });
      ...
      if (!token) {
        error3 = "GitHub did not return a token: " + (data.error_description || "unknown error");
      }
    } catch (e) {
      error3 = "Could not reach GitHub: " + e.message;
    }
  }
  const content = token ? "authorization:github:success:" + JSON.stringify({ token, provider: "github" }) : "authorization:github:error:" + JSON.stringify({ message: error3 });
...
// ../.wrangler/tmp/pages-wwhIBv/functionsRoutes-0.10564062964051268.mjs
var routes = [
  {
